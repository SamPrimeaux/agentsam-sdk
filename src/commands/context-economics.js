...
export function buildContextEconomicsReport(cwd, options = {}) {
  ...
  return Object.freeze({
    ...
    model_key: model.model_key,
    resolved: true,
    compaction_supported: ['openai', 'grok'].includes(model.provider) && model.capabilities?.compaction === true,
...
export function renderContextEconomics(report) {
  ...
  return [
    ...
    '',
    '  Compaction',
    `    support         ${report.compaction_supported ? 'native' : 'unavailable'}`,
    `    automatic       ${report.compaction_supported && report.auto_compact ? 'enabled' : 'disabled'}`,
    `    threshold       ${formatInteger(report.compact_at_tokens)}`,
    '    command         /compact · /compact status',
    '',
    '  Economics',
