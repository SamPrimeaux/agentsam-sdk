...
const operationToProto = { index: knowledgePb.JobOperation.JOB_OPERATION_INDEX, plan: knowledgePb.JobOperation.JOB_OPERATION_PLAN, search: knowledgePb.JobOperation.JOB_OPERATION_SEARCH };
const operationFromProto = new Map(Object.entries(operationToProto).map(([key, value]) => [value, key]));
const statusToProto = { queued: knowledgePb.JobStatus.JOB_STATUS_QUEUED, running: knowledgePb.JobStatus.JOB_STATUS_RUNNING, completed: knowledgePb.JobStatus.JOB_STATUS_COMPLETED, failed: knowledgePb.JobStatus.JOB_STATUS_FAILED };
...
const httpToGrpc = new Map([[400, grpc.status.INVALID_ARGUMENT], [401, grpc.status.UNAUTHENTICATED], [403, grpc.status.PERMISSION_DENIED], [404, grpc.status.NOT_FOUND], [409, grpc.status.ALREADY_EXISTS], [413, grpc.status.RESOURCE_EXHAUSTED], [415, grpc.status.INVALID_ARGUMENT], [429, grpc.status.RESOURCE_EXHAUSTED], [503, grpc.status.UNAVAILABLE]]);
const grpcToHttp = new Map([[grpc.status.CANCELLED, 499], [grpc.status.INVALID_ARGUMENT, 400], [grpc.status.DEADLINE_EXCEEDED, 504], [grpc.status.NOT_FOUND, 404], [grpc.status.ALREADY_EXISTS, 409], [grpc.status.PERMISSION_DENIED, 403], [grpc.status.RESOURCE_EXHAUSTED, 429], [grpc.status.FAILED_PRECONDITION, 412], [grpc.status.ABORTED, 409], [grpc.status.UNAUTHENTICATED, 401], [grpc.status.UNAVAILABLE, 503]]);

function timestamp(value) { const message = new Timestamp(); message.fromDate(new Date(value)); return message; }
function iso(message) { return message ? message.toDate().toISOString() : null; }
function jobId(value) { const id = new knowledgePb.JobId(); id.setValue(value || ''); return id; }
...
export function decodeJob(message) {
  ...
  return { id: message.hasId() ? message.getId().getValue() : '', status: statusFromProto.get(message.getStatus()) || 'unknown', attempts: message.getAttempts(), created_at: iso(message.getCreatedAt()), updated_at: iso(message.getUpdatedAt()), result: resultJson ? JSON.parse(resultJson) : null, error: failure ? failure.getMessage() : null };
}

export function encodeRepositoryList(value) {
  const response = new knowledgePb.ListRepositoriesResponse();
  response.setRepositoriesList(value.repositories.map(repo => { const ref = new knowledgePb.RepositoryRef(); ref.setAlias(repo.alias); ref.setRepositoryId(repo.repository_id || ''); return ref; }));
...
export function encodeSubmitRequest(body, idempotencyKey) {
  ...
  if (body?.include !== undefined) { const list = new knowledgePb.StringList(); list.setValuesList(body.include); message.setInclude(list); }
  if (body?.exclude !== undefined) { const list = new knowledgePb.StringList(); list.setValuesList(body.exclude); message.setExclude(list); }
...
export function encodeGetJobRequest(id, RequestType = knowledgePb.GetJobRequest) { const request = new RequestType(); request.setJobId(jobId(id)); return request; }
export function decodeJobId(request) { return request.hasJobId() ? request.getJobId().getValue() : ''; }
export function encodeJobEvent(job, sequence) { const event = new knowledgePb.JobEvent(); event.setJob(encodeJob(job)); event.setObservedAt(timestamp(new Date().toISOString())); event.setSequence(sequence); return event; }
export function decodeJobEvent(event) { return { job: decodeJob(event.getJob()), observed_at: iso(event.getObservedAt()), sequence: event.getSequence() }; }

export function metadataForToken(token) { if (!token) throw new Error('A service token is required.'); const metadata = new grpc.Metadata(); metadata.set('authorization', `Bearer ${token}`); return metadata; }
export function authorizationFromMetadata(metadata) { const values = metadata?.get('authorization') || []; return values.find(entry => typeof entry === 'string') || ''; }
...
export function toGrpcError(error) {
  ...
  return Object.assign(new Error(safeMessage), { code: httpToGrpc.get(error?.status) ?? grpc.status.INTERNAL, details: safeMessage, metadata });
}
...
export function fromGrpcError(error) {
  ...
  return Object.assign(new Error(message), { status: grpcToHttp.get(error?.code) ?? 500, grpcCode: error?.code, rpcCode: detail ? errorCodeNames.get(detail.getCode()) || 'INTERNAL' : undefined, retryable: detail?.getRetryable() || false });
}
