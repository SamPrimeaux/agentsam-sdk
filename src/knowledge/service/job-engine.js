...
export function createKnowledgeJobEngine({ stateDir, repositories, allowEmbeddings = false, maxQueued = 32, maxFiles = 2000, jobTimeoutMs = 600000, workerPath = defaultWorkerPath } = {}) {
  ...
  const updateStatus = (id, status, { result = null, error = null } = {}) => {
    db.prepare('UPDATE jobs SET status=?,result=?,error=?,updated_at=? WHERE id=?').run(status, result === null ? null : JSON.stringify(result), error, new Date().toISOString(), id);
    const job = getRaw(id);
    publish(job);
    return job;
  };
  ...
  const api = {
    listRepositories() {
      return { repositories: Object.entries(registry).map(([alias, repo]) => ({ alias, repository_id: repo.config.repository_id })), embeddings_enabled: allowEmbeddings };
    },
    getJob(id) {
