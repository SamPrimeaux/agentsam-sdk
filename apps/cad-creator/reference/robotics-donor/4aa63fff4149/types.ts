...
/**
 * Real-time simulation telemetry and diagnostics
 */
export interface SimulationTelemetry {
  ...
  gripperForce: number; // in Newtons
  gripperCtrl: number;
  jointVelocities: number[]; // rad/s for joints 1-7
  jointPositions: number[];  // rad for joints 1-7
  ...
  endEffectorVel: number; // m/s
  simulationStatus: 'running' | 'paused' | 'picking' | 'ik_active' | 'idle';
}

/**
 * Tool integration status and descriptor
 */
export interface ToolIntegrationStatus {
  id: 'blender' | 'openscad' | 'freecad' | 'meshy';
  name: string;
  category: 'deterministic_cad' | 'renderer' | 'solid_kernel' | 'generative_ai';
  status: 'ready' | 'connected' | 'simulated' | 'exporting' | 'synced';
  description: string;
