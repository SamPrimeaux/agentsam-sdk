  ...
  return (
    <article className="group flex min-h-52 flex-col rounded-2xl bg-card p-5 shadow-hairline transition-[box-shadow,transform] duration-150 hover:-translate-y-0.5 hover:shadow-[0_0_0_1px_color-mix(in_oklab,var(--color-accent)_32%,transparent)]">
      <button
        ...
        onClick={onOpen}
        aria-label="Open Cloudflare connection details"
      >
        ...
        <div className="mt-6">
          <h2 className="text-lg font-medium tracking-tight text-foreground">Cloudflare</h2>
          <p className="mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">
            Authorize Local Studio to work with the Cloudflare account you choose.
          </p>
        </div>
      </button>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-border pt-4">
        <span className="text-xs text-muted-foreground">
          {connected ? "OAuth authorization stored" : "OAuth with PKCE"}
        </span>
        {connected ? (
