...
// Tripwires: the deployed-app path only works if Nitro scans backend/server/ — an
...
test("nitro middleware and its bundled assets exist", () => {
  const middleware = readFileSync(
    join(TEMPLATE_ROOT, "backend/server/middleware/grok-pwa.ts"),
    "utf8",
  );
  assert.match(middleware, /install-page\.html\?raw/);
  assert.match(middleware, /virtual:grok-og-identity/);
  readFileSync(join(TEMPLATE_ROOT, "scripts/install-page.html"));
  readFileSync(join(TEMPLATE_ROOT, "frontend/public/__grok/icon-180.png"));
  readFileSync(join(TEMPLATE_ROOT, "frontend/public/__grok/install/styles.css"));
});
