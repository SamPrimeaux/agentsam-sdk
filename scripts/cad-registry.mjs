...
statements.push(
 "UPDATE agentsam_products SET metadata=json_set(metadata,'$.capabilities',json("+quote(json(product.capability_keys))+"),'$.cli_commands',json("+quote(json(product.cli_commands))+"),'$.cad_project_contract','cad.project.v1'), updated_at=unixepoch() WHERE slug="+quote(product.slug)+";"
);
for(const t of tools){
 statements.push(
